...
def main():
    ...
    for paint_line in reversed(answer):
...
def solve_part_2(program):
    visited = set()
    hull = Hull()
    robot = EmergencyHullPaintingRobot(hull)
    robot.load_program(program)
    ...
    x_visited, y_visited = [x for x, y in visited], [y for x, y in visited]
    ...
    for j in range(y_min, y_max + 1):
        ...
        for i in range(x_min, x_max + 1):
            point_painting = hull.get_value((i, j))
            line_painting.append(point_painting)
        hull_painting.append(line_painting)

    return hull_painting
...
class EmergencyHullPaintingRobot:
    ...
    def load_program(self, program):
        self._computer.load_program(program)

    def step(self):
        c = self._hull.get_value(self.position)
        self._computer.write_to_input(c)

        while not self._computer.is_output_available():
            self._computer.step()
            if self._computer.has_halted():
                return
        paint_colour = self._computer.read_output()
        self._hull.set_value(self.position, paint_colour)

        while not self._computer.is_output_available():
            self._computer.step()
            if self._computer.has_halted():
                return
        turn_direction = self._computer.read_output()
        self._turn(turn_direction)

        self._step_forward()

    def has_halted(self):
        return self._computer.has_halted()

    def _turn(self, turn_direction):
        direction_circle = [(0, 1), (1, 0), (0, -1), (-1, 0)]
        ...
        current_i = direction_circle.index(self.direction)
        new_i = (current_i + delta) % len(direction_circle)

        new_direction = direction_circle[new_i]
        self.direction = new_direction

    def _step_forward(self):
        self.position = add_pairs(self.position, self.direction)
...
class Hull:
    ...
    def get_value(self, p):
        if p in self._painted:
            return self._get_painted_value(p)
        else:
            return self._get_unpainted_value(p)

    def _get_painted_value(self, p):
        return self._d[p]

    def _get_unpainted_value(self, p):
        if p == (0, 0):
            return 1
        else:
            return 0
