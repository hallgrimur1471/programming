...
def main():
    ...
    answer = solve_part_2(program)

    print(f"There are score after the last block is broken is {answer}")
...
def solve_part_1(program):
    ...
    num_blocks = 0
    while arcade.is_output_available():
        x = arcade.read_output()
        y = arcade.read_output()
        tile_id = arcade.read_output()
        if tile_id == 2:
            num_blocks += 1
...
def solve_part_2(program):
    ...
    while not arcade.is_halted():
        ...
        if arcade.get_ball_x_position() < arcade.get_paddle_x_position():
            arcade.set_joystick("left")
        elif arcade.get_ball_x_position() > arcade.get_paddle_x_position():
            arcade.set_joystick("right")
        else:
            arcade.set_joystick("neutral")

        print("ball   x:", arcade.get_ball_x_position())
        print("paddle x:", arcade.get_paddle_x_position())

    return arcade.get_score()


class Arcade(object):
    def __init__(self):
        self._computer = IntcodeComputer()

        self._ball_x_position = None
        self._paddle_x_position = None
        self._score = None

    def load_game(self, game):
        self._computer.load_program(game)

    def step(self):
        self._computer.run_until_next_input()
        if not self._computer.is_halted():
            outputs = self._computer.read_all_output()
            self._update_knowledge_of_game(outputs)

    def is_halted(self):
        return self._computer.is_halted()

    def set_joystick(self, position):
        if position == "neutral":
            self._computer.write_to_input(0)
        elif position == "left":
            self._computer.write_to_input(-1)
        elif position == "right":
            self._computer.write_to_input(1)
        else:
            raise RuntimeError(
                f"'{position}' is an unsupported joystick position"
            )

    def get_ball_x_position(self):
        return self._ball_x_position

    def get_paddle_x_position(self):
        return self._paddle_x_position

    def get_score(self):
        return self._score

    def _update_knowledge_of_game(self, outputs):
        outputs = deque(outputs)

        while outputs:
            x = outputs.popleft()
            y = outputs.popleft()
            object_id = outputs.popleft()

            if object_id == 3:
                self._paddle_x_position = x
            elif object_id == 4:
                self._ball_x_position = x

            if x == -1 and y == 0:
                self._score = object_id


# class GameBoard:
#    def __init__(self):
#        self._pos_2_object = dict()
#
#    def set_object(self, position, object_):
#        self._pos_2_object[position] = object_
#
#    def get_object(self, position):
#        return self._position_2_object[position]
...
class IntcodeComputer:
    ...
    def run_until_next_input(self):
        if self._current_instruction_is_input():
            self.step()
        while not self.is_halted() and not self._current_instruction_is_input():
            self.step()
    ...
    def read_all_output(self):
        outputs = []
        while self.is_output_available():
            outputs.append(self.read_output())
        return outputs

    def is_halted(self):
        return self._halt

    def _current_instruction_is_input(self):
        instruction = self._program_parser.parse_instruction(
            self._program, self._instruction_pointer
        )
