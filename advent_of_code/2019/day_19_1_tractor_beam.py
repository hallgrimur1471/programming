...
def main():
    ...
    print(f"{answer} points are affected by the tractor beam")
...
def solve_part_1(program):
    ...
    for x in range(50):
        for y in range(50):
            drone = IntcodeComputer()
            drone.load_program(program)

            drone.write_to_input(x)
            drone.write_to_input(y)

            while not drone.is_halted() and not drone.is_output_available():
                drone.step()

            if drone.is_output_available():
                output = drone.read_output()
                map_[(x, y)] = output
