...
CASES = (list(range(1, 100 + 1)), [1, 2, 3])  # fill in your own cases
QS = (365, 7)
...
def RunCase(qs, mg, case, test_input=None):
    print("Num Gophers: {}".format(case), file=sys.stderr, flush=True)
    outputs = []
...
def RunCases(qs, mg, cases):
    ...
    return "Additional input after all cases finish: {}".format(
        extra_input[:100]
    )
